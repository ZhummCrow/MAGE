...
class plan1_tgt_Package(nn.Module):
    def __init__(self,
                 edge_width            ,
                 num_heads             ,
                 attention_dropout = 0 ,
                 ):
        super(plan1_tgt_Package, self).__init__()
    ...
    def forward(self,h_V,subgraph_mask,batch_id):
        '''
            sub_graph_mask: 这里用mut切出来的子图, 其中长度与蛋白质长度一致, mask=1的位置表示子图
            sub_graph_mask要求必须为torch.bool

            step 1. 切子图, 转batch --> [B,subgraph L, hid]
            step 2. 爱因斯坦乘  --> [B, subgraph L, subgraph L, hid]
            step 3. 过tgt, 并降维, 切换回dense --> [B*subgraph L, hid]
            step 4. 将更新后的subgraph, 过残差过ffw, 替换回h V
        '''
        assert subgraph_mask.dtype == torch.bool, "subgraph_mask must be torch.bool"
        BL,hid_dim = h_V.shape
        
        # step 1.
        sub_h_V = h_V[subgraph_mask] # [B*subgraph L, hid] 子图的dense表示
        sub_batch_id = batch_id[subgraph_mask] # [B*subgraph L, ]

        sub_h_V_batch,sub_h_V_batch_mask = to_dense_batch(sub_h_V,sub_batch_id) # [B,subgraph L, hid]  [B, subgraph L] 子图的batch表示
        ...
        subgraph_edge_mask = subgraph_edge_mask.unsqueeze(-1).to(torch.float) # 为了运算能广播操作，要最后增加一个维度
        subgraph_edge_mask_tgt = (1-subgraph_edge_mask)*torch.finfo(subgraph_edge_mask.dtype).min # 提前置mask为极小

        # step 3.
        subgraph_edge = self.tgt(subgraph_edge,subgraph_edge_mask_tgt) # [B, subgraph L, subgraph L, hid]

        subgraph_edge = subgraph_edge.masked_fill(subgraph_edge_mask == 0, 0)  # 为了后面mean pooling的时候没错
        ...
        # 至此完成tgt降维-->子图的batch表示-->子图的dense表示


        # step 4.
        sub_h_V = self.norm[0](sub_h_V + self.drop(dh)) # 和最开始的子图做一个残差
        
        # Position-wise feedforward
        dh = self.PositionWiseFeedForward(sub_h_V)
        sub_h_V = self.norm[1](sub_h_V + self.drop(dh))

        h_V_new = h_V.clone()  # 创建副本
        h_V_new[subgraph_mask] = sub_h_V  # 修改副本
        h_V = h_V_new  # 重新赋值（非原地操作）
...
class Graph_encoder(nn.Module):
    def __init__(self, node_in_dim, edge_in_dim, hidden_dim,
        ...
        self.tgt_layers = nn.ModuleList([plan1_tgt_Package(edge_width=hidden_dim,num_heads=4,attention_dropout=drop_rate) for _ in range(num_layers)])
    ...
    def forward(self, h_V, edge_index, h_E, batch_id,subgraph_mask):
        ...
        for gnn_layer,tgt_layer in zip(self.gnn_layers,self.tgt_layers):
            ...
            h_V = tgt_layer(h_V,subgraph_mask,batch_id) # 这里同时完成切图，过tgt，再替换hv切下来的部分，复原hv
        
        return h_V



class SelfAttention_cross(nn.Module):
...
class GPSite(nn.Module): # Geometry-aware Protein Sequence-based binding site predictor
    def __init__(self, node_input_dim, edge_input_dim, hidden_dim, num_layers, dropout, task, num_att_layers,num_tgt_layers=2):
        ...
        self.cross_attention_layers =  nn.ModuleList([SelfAttention_cross(hid_dim=hidden_dim,n_heads=1,dropout=dropout) for _ in range(num_att_layers)])
    ...
    def forward(self, wt_data, mut_data, smiles):
        # X, h_V, edge_index, seq, batch_id, smiles
        X = wt_data.X
        ...
        delta_h_V = mut_h_V - wt_h_V  # 得到delta hv,  [B*L,128]
        ...
        delta_h_V = torch.cat([
            global_max_pool(delta_h_V,batch),global_mean_pool(delta_h_V,batch), 
            delta_smiles,
            smiles,
        ],dim=-1)
