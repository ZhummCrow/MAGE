...
def find_x_positions(string):
    positions = []
    wrongs= ['B','J','U','X','Z']
    for t in wrongs:
        start = 0
        while True:
            index = string.find(t, start)
            if index == -1:
                break
...
class ProteinGraphDataset(data.Dataset):
    def __init__(self, config):
        ...
        self.X = torch.load(config['structs_path']+self.protein_name+".tensor")
    ...
    def _featurize_graph(self, idx):
        ...
        mutant = row['mutant'] # 唯一标识一条突变数据

        with torch.no_grad():

            mut_esm = torch.load(self.esm_path + mut_seq_id + ".tensor")
            mut_pre_computed_node_feat = torch.cat([mut_esm, self.DSSP], dim=-1)  
            
            wt_esm = torch.load(self.esm_path+ self.protein_name + ".tensor")
        ...
        wt_graph_data = torch_geometric.data.Data(name=mutant,  node_feat=wt_pre_computed_node_feat, 
                                                X=self.X, edge_index=self.edge_index)
        mut_graph_data = torch_geometric.data.Data(name=mutant, node_feat=mut_pre_computed_node_feat,
                            mut_graph_mask10=mut_graph_mask10,mut_graph_mask12=mut_graph_mask12)
        
        return wt_graph_data, mut_graph_data, self.smiles_feat
